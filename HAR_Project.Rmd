---
title: "A predictive model on Human Activity Recognition"
author: "Junhui Cai"
...
pmltest_c <- pmltest_c[8:length(pmltest_c)]
...
### Model comparision
As I pointed out earlier, we can use variety of different algorithms in this type of classification problem. Which algorithm is the best usually depend on the dataset. In this project, I will try following algorithm:
...
The Random Forest algorithm in caret package run significant slower than the base RandomForest() since caret has different model selection and accuracy estimation.  The following is comparision of the results of 4 different algorithms:

```{r}
results <- data.frame(
  Algorithm = c("Random Forest", "Random Forest in caret", "GBM", "SVM"), 
  CV_Accuracy = c("99.4%", "99.3%", "96%","95%"), 
  Correct_in_20_Problems = c(20,20,20,19),
...
The comparision table shows the Random Forest algorithm get the best result, GBM and SVM are not as good. This is as expected because Random Forest is the "Combined Classifier", which get the majority votes of multiple runs. 
